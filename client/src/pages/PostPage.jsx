...
export default function PostPage() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        setLoading(true);
        const res = await fetch(`${process.env.VITE_API_URL}/api/post/getposts?slug=${postSlug}`);
  ...
  useEffect(() => {
    try {
      const fetchRecentPost = async () => {
        const res = await fetch(`${process.env.VITE_API_URL}/api/post/getposts?limit=3`);
        const data = await res.json();
        if (res.ok) {
